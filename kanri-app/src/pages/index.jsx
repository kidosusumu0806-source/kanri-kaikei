...
export function CostClassifier({ costs, onChange, journals }) {
  ...
  const upd = (i, field, val) => { const n=[...costs]; n[i]={...n[i],[field]:val}; onChange(n); };
          ...
          {costs.map((c,i) => {
            ...
            return (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 12px", background:C.bgL, borderRadius:8, border:`1px solid ${C.b}` }}>
                ...
                {c._type==="準変動費" && (
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    <input type="range" min={0} max={100} step={5} value={c.固定率??60} onChange={e=>upd(i,"固定率",parseInt(e.target.value))}
